...
public class LottoEvent implements ItemListener, ActionListener, Runnable 
{
    ...
    public void actionPerformed(ActionEvent event)
    {
        ...
        if (command.equals("Play")) startPlaying();
        if (command.equals("Stop")) stopPlaying();
        if (command.equals("Reset")) clearAllFields();
    }
    ...
    public void itemStateChanged(ItemEvent event)
    {
        ...
        {
            for (int i = 0; i < 6; i++) {gui.numbers[i].setText(null);}
        }
    }
    ...
    boolean numberGone(int num, JTextField[] pastNums, int count)
    {
      ...
      {
          if ((Integer.parseInt(pastNums[i].getText())) == num) {return true;}
      }
      return false;
    }
    ...
    boolean matchedOne(JTextField win, JTextField[] allPicks)
    {
        for (int i=0; i<6; i++)
        {
            ...
            if (winText.equals(allPicks[i].getText())) {return true;}
        }
        return false;
    }
    ...
    public void run()
    {
        ...
        while (playing == thisThread)
        {
            ...
            {
                int ball;
                do {ball = (int) Math.floor(Math.random() * 50 + 1);}
                while (numberGone(ball, gui.winners, i));
                gui.winners[i].setText("" + ball);
                if (matchedOne(gui.winners[i],gui.numbers)) matches++;
